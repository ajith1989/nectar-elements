 ...
 * Counter component
 */
export const Counter: FC<CounterProps> = ({ icon, label, count, theme = 'primary', border = true, className, style, onClick, loading = false, skeleton = false }) => {
    ...
    if (skeleton) {
        return (
            <div className={`flex items-center space-x-4 ${border ? "mb-6 shadow-xl shadow-primary/10" : "mb-0"} py-4 px-8 rounded-lg hover:bg-primary/10 ${className}`} style={style} onClick={onClick}>
                <div className={`h-12 w-12 animate-pulse rounded-md ${getButtonTheme(theme)}`} />
                <div className="flex flex-col">
                    <div className={`h-4 w-12 animate-pulse rounded-md ${getButtonTheme(theme)}`} />
